...
# Three stale expectations from earlier accepted refactors.
replace(
...
p.write_text(text[:start] + chunk + text[end:], encoding="utf-8")

p = ROOT / "tests/coordinator/test_valve_protection.py"
text = p.read_text(encoding="utf-8")
old_fn = '''    async def test_valve_protection_runs_when_climate_off(self, hass, mock_config_entry):\n        """Valve protection works even when climate_control_active is False."""\n'''
new_fn = '''    async def test_valve_protection_does_not_run_when_climate_off(self, hass, mock_config_entry):\n        """Climate Control OFF prevents starting new valve-protection cycles."""\n'''
if old_fn not in text:
    raise SystemExit("valve protection function anchor not found")
text = text.replace(old_fn, new_fn, 1)
fn_start = text.index("    async def test_valve_protection_does_not_run_when_climate_off")
fn_end = text.index("\n    @pytest.mark.asyncio", fn_start)
fn = text[fn_start:fn_end]
old_assert = '        assert "climate.living_room" in coordinator._valve_manager._cycling\n'
if old_assert not in fn:
    raise SystemExit("valve protection assertion anchor not found")
fn = fn.replace(old_assert, '        assert "climate.living_room" not in coordinator._valve_manager._cycling\n', 1)
